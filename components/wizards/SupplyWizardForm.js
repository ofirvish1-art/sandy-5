...
import PublisherDeclaration from "@/components/PublisherDeclaration";
import { LEGAL_CONTENT } from "@/lib/legalContent";

const TOTAL_STEPS = 4;

// The actual "יש לי לתת" wizard — shared between the standalone /supply page
// (bookmarkable, used for editing via ?edit=id) and the instant bottom-sheet
// modal opened from Home / the bottom nav's FAB. Owns all form state; the
// caller only supplies editId and an onSuccess callback for what happens
// after a successful publish.
export default function SupplyWizardForm({ editId, onSuccess }) {
  ...
  const [declared, setDeclared] = useState(LEGAL_CONTENT.publisherDeclaration.map(() => false));
  function toggleDeclared(i) {
    setDeclared((d) => d.map((v, idx) => (idx === i ? !v : v)));
  }
  ...
  const stepValid = [
    ...
    (!!form.image || (!!editId && !!existingImageUrl)) && declared.every(Boolean), // mandatory photo/video + declaration, unless editing and one's already saved
  ][step];
  ...
  if (loadingExisting) {
    return <p className="text-forest/50">טוען מודעה…</p>;
  }
          ...
          <>
            <div>
              ...
              <select className="field-input" value={form.transport} onChange={(e) => update("transport", e.target.value)}>
                <option value="buyerPickup">אתה תיקח</option>
            ...
            <PublisherDeclaration checked={declared} onToggle={toggleDeclared} />
          </>
        )}
      </Wizard>
